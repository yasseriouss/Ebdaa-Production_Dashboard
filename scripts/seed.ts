...
const metalOrdersData = [
  ...
  { moNumber: "MO-2025-005", project: "مشروع الكيان العسكري - المرحلة 2", client: "الكيان العسكري", product: "خزائن تبريد", qty: "120", unit: "قطعة", deliveredQty: "80", completionPct: "67", backlogQty: "40", backlogStatus: "في المخزن", notes: "", status: "في المخزن" },
  ...
  { moNumber: "MO-2025-010", project: "طلب عام", client: "عملاء متنوعون", product: "قواطع معدنية", qty: "1000", unit: "قطعة", deliveredQty: "600", completionPct: "60", backlogQty: "400", backlogStatus: "جاري", notes: "توريد دوري", status: "تحت التصنيع" },
];
...
const woodenOrdersData = [
  ...
  { orderNo: "WO-2025-004", extension: "D3", orderDate: "2025-02-15", manufactureRequest: "MR-004", sapCode: "SAP-004", client: "الحرس الجمهوري", subProject: "قاعة اجتماعات", product: "طاولات مؤتمرات خشبية", category: "طاولات", uom: "طاولة", qty: "20", done: "20", rem: "0", status: "Delivered", prodDateStart: "2025-02-20", prodDateEnd: "2025-04-20", prodDateFinished: "2025-04-15" },
  { orderNo: "WO-2025-005", extension: "A1", orderDate: "2025-03-01", manufactureRequest: "MR-005", sapCode: "SAP-005", client: "الكيان العسكري", subProject: "مبنى ضباط", product: "خزائن ملابس خشبية", category: "خزائن", uom: "قطعة", qty: "80", done: "40", rem: "40", status: "Production", prodDateStart: "2025-03-10", prodDateEnd: "2025-06-10", prodDateFinished: "" },
...
async function seed() {
  ...
  if (existing.length > 0) {
    console.log(`Already seeded (${existing.length} metal orders exist). Skipping.`);
    process.exit(0);
  }

  // Insert metal work orders
  for (const orderData of metalOrdersData) {
    ...
    const stagesToInsert = METAL_STAGES.map((s, i) => {
      ...
      return {
        ...
        stageName: s.name,
        stageOrder: s.order,
        qtyTarget: orderData.qty,
        qtyDone: String(qtyDone),
        status,
      };
    });
  ...
  console.log(`Inserted ${metalOrdersData.length} metal work orders with stages`);

  // Insert wooden work orders
  ...
  console.log(`Inserted ${woodenOrdersData.length} wooden work orders with stages`);
