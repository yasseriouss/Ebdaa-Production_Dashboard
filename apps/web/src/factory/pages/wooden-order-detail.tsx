...
import { getWoodenStatusBadgeClass } from "@factory/data/woodenOrderStatuses";
  ...
  if (!order) {
    return (
      <div className="text-center py-16 text-muted-foreground">
        ...
        <Link href="/orders/wood"><Button variant="link" className="mt-2">{ft("orderDetail.backToList")}</Button></Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 flex-wrap">
        <Link href="/orders/wood">
          <Button variant="ghost" size="sm" data-testid="btn-back-wooden"><ArrowRight className="h-4 w-4 ml-1" />{ft("orderDetail.back")}</Button>
        </Link>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">{ft("orderDetail.client")}</CardTitle></CardHeader><CardContent><div className="font-semibold">{order.client || "—"}</div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">{ft("orderDetail.subProject")}</CardTitle></CardHeader><CardContent><div className="font-semibold">{order.subProject || "—"}</div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">{ft("orderDetail.product")}</CardTitle></CardHeader><CardContent><div className="font-semibold">{order.product}</div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">{ft("orderDetail.qtyTotal")}</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{order.qty} <span className="text-sm text-muted-foreground">{order.uom}</span></div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">{ft("orderDetail.done")}</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-green-400">{order.done || 0}</div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">{ft("orderDetail.remaining")}</CardTitle></CardHeader><CardContent><div className={`text-2xl font-bold ${rem > 0 ? "text-yellow-400" : "text-green-400"}`}>{rem}</div></CardContent></Card>
      </div>
