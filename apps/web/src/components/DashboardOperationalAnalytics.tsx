...
import { AnalyticsMetricRow } from "./dashboard/AnalyticsMetricRow";
...
import type { EmployeeAssignments, FactoryCapacitySchema, WoodWorkOrder } from "../data/types";
...
const PRODUCTIVITY_WEEK_BASE: Array<{
...
/** Reference / fixture-based operational charts moved from home dashboard to analytics. */
export function DashboardOperationalAnalytics() {
  ...
  return (
    <section
      id="operational-analytics-embedded"
      className="animate-in fade-in space-y-6 duration-300"
      aria-labelledby="operational-analytics-heading"
    >
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-brand-border/40 bg-brand-elevated/30 px-4 py-3 text-sm text-muted-foreground">
        <p className="max-w-2xl leading-relaxed">{t("dashboard.operationalRelocatedHint")}</p>
        <Link
          href="/"
          className="shrink-0 rounded-md border border-brand-border px-3 py-1.5 text-xs font-bold text-brand-luxury hover:bg-brand-border/20 focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-wood"
        >
          {t("dashboard.backToExecutiveHome")}
        </Link>
      </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <XAxis
                  ...
                  tickMargin={10}
                />
                <YAxis stroke={chart.axis} fontSize={10} tickLine={false} axisLine={false} width={52} tickMargin={16} />
