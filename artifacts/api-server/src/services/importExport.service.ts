      ...
      try {
        const [order] = await db.insert(metalWorkOrdersTable).values(orderValues)
          .onConflictDoUpdate({
            ...
            set: { product: orderValues.product, qty: orderValues.qty, client: orderValues.client, project: orderValues.project, unit: orderValues.unit, updatedAt: new Date().toISOString() },
          })
          .returning();
        ...
        try {
          ...
          if (matchedStage) {
            await db
              ...
              .set({ qtyDone: String(qty), status: statusFinal })
...
router.post("/wooden-orders", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    for (let i = headerRow + 1; i < data.length; i++) {
      ...
      const orderDateStr = rawDate ? excelDateToStr(rawDate) || safeStr(rawDate) : "";
      const prodDateEndStr =
        prodDateEndIdx >= 0 ? excelDateToStr(row[prodDateEndIdx]) || safeStr(row[prodDateEndIdx]) : "";
      ...
      const woodenRow = {
        ...
        prodDateEnd: prodDateEndStr || null,
      };

      try {
        const [order] = await db.insert(woodenWorkOrdersTable).values(woodenRow)
          .onConflictDoUpdate({
            target: woodenWorkOrdersTable.orderNo,
            set: {
              product: woodenRow.product,
              qty: woodenRow.qty,
              done: woodenRow.done,
              rem: woodenRow.rem,
              client: woodenRow.client,
              subProject: woodenRow.subProject,
              status: woodenRow.status,
              prodDateEnd: woodenRow.prodDateEnd,
            },
          })
          .returning();
...
async function importMetalSection(rows: unknown[][]): Promise<SectionResult> {
  ...
  for (let i = 2; i < rows.length; i++) {
    ...
    try {
      ...
      const backlogNote = get(row, "backlog_status");
      const baseNotes = get(row, "notes");
      const mergedNotes = [baseNotes, backlogNote].filter(Boolean).join(" | ") || undefined;
      ...
      for (const s of METAL_STAGES) {
        ...
        await db.insert(metalProductionStagesTable).values({
          ...
          target: [metalProductionStagesTable.metalOrderId, metalProductionStagesTable.stageName],
          set: { qtyTarget: qty, qtyDone, status: stageStatus, stageOrder: s.order },
        });
      }
      result.rowsImported++;
    } catch (e) {
  ...
  for (let i = 2; i < rows.length; i++) {
    ...
    try {
      ...
      const [order] = await db.insert(woodenWorkOrdersTable).values({
        ...
        prodDateEnd: getDate(row, "prod_date_end") || null,
      }).returning();
...
async function importStageLogSection(rows: unknown[][]): Promise<StageLogResult> {
  ...
  const orderIdCache = new Map<string, string | null>();
...
router.get("/metal-orders", async (req, res) => {
  try {
    ...
    if (dateFrom) {
      ...
      if (!isNaN(from)) {
        orders = orders.filter(
          o => o.createdAt && !Number.isNaN(new Date(o.createdAt).getTime()) && new Date(o.createdAt).getTime() >= from,
        );
      }
    }
    if (dateTo) {
      ...
      if (!isNaN(to)) {
        orders = orders.filter(
          o => o.createdAt && !Number.isNaN(new Date(o.createdAt).getTime()) && new Date(o.createdAt).getTime() <= to,
        );
      }
    }
    ...
    if (format === "xlsx") {
      ...
      const stagesByOrder = new Map<string, Map<string, string>>();
      ...
      for (const o of orders) {
        ...
        wsData.push([
          ...
          ...stageCells,
          o.completionPct || "0", o.backlogQty || "0", "",
          o.status || "", o.notes || "", o.deliveredQty || "0", "metal", "",
        ]);
      }
  ...
  try {
    ...
    if (format === "xlsx") {
      ...
      for (const o of orders) {
        ...
        wsData.push([
          o.orderNo, o.extension || "", o.orderDate || "", "", "",
          o.client || "", o.subProject || "", o.product, "", "",
